...
class ProductPricesController < ApplicationController
  ...
  def edit
    # @product = Product.find_by_product_price(params[:id])
    @product_price = ProductPrice.find(params[:id])
  ...
  def update
    ...
    respond_to do |format|
      #zaktualizuj ceny produktu
      if params[:actualPriceOnPurchase].nil?
        actualPriceOnPurchase = false;
      else
        actualPriceOnPurchase = true;
      end

      if !@product.update_attributes(defaultVat: params[:defaultVat], discount: params[:discount], actualPriceOnPurchase: actualPriceOnPurchase )
      ...
      if @product_price.update_attributes(params[:product_price])
        ...
        format.html { redirect_to products_path, notice: 'Pomyślnie zaktualizowano ceny.' }
